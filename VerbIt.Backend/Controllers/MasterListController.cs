...
public class MasterListController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<MasterListRow[]>> Edit(Guid listId, EditMasterListRequest editRequest, CancellationToken token)
    {
        EditMasterListRowRequest? invalidRow = editRequest.Rows?.FirstOrDefault(x => x.RowNum == null && x.Words == null);
        if (invalidRow != null)
        {
            return BadRequest($"All rows must have at either a RowNum or a Words array. RowId {invalidRow.RowId} had neither.");
        }
